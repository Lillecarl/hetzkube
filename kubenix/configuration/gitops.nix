    ...
    { ignoreDifferences ? [ ] }:
    {
      ...
      spec = {
        ...
        source = {
          ...
          targetRevision = config.gitOps.deployBranch;
          inherit path;
        };
        destination = {
...
in
{
  config = lib.mkIf (config.stage == "full") {
    gitOps = {
      enable = true;
      deployBranch = "deploy";
      # Every deploy commit also carries a paired commit on this branch
      # with the exact source tree (including uncommitted edits) that
      # produced it -- see ekn/src/ekn/git.py's
      # prepare_deploy_and_source_commits/finalize_branches.
      sourceBranch = "source";
      targets = {
